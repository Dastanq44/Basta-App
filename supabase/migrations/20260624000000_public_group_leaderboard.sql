-- Public group leaderboard — the member-only `group_leaderboard` raises for non-members, so the
-- public group preview needs a version gated on `get_group_access` (member OR public preview). Same
-- shape/ordering; member names + verified counts of a PUBLIC group are not private. No private data.
