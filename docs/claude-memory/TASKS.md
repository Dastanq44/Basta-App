...
| T-050B | DONE | Server-side dispatch for verification-flow pushes. `notification_outbox` table (pending → processing → sent/failed lifecycle, attempts counter); AFTER INSERT trigger on `submissions` enqueues `verify_needed` for each eligible group member (excludes author + blocked pairs + dedups against existing pending); AFTER UPDATE OF status trigger enqueues `verify_result` for the author when status flips to verified/rejected. Four dispatch RPCs (`claim_pending_notifications`, `mark_notification_sent`, `mark_notification_failed`, `revoke_push_token`) restricted to `service_role` only — PUBLIC/anon/authenticated revoked. W-032 register/unregister RPCs also hardened (revoke from PUBLIC + explicit grant to authenticated). Supabase Edge Function `supabase/functions/dispatch-pushes/index.ts`: claims up to 50 pending rows, batches Expo Push Service calls at 100/req, handles ok/error tickets + DeviceNotRegistered (revokes the token) + retryable codes (MessageRateExceeded etc.), maps HTTP 429/5xx → retryable + 4xx → terminal. Returns `{claimed, sent, failed, retryable, revokedTokens}` summary. Migration `20260607000000_notification_outbox_and_dispatch.sql` (W-033 — USER must apply + `supabase functions deploy`). NOT in slice: prefs UI, quiet hours, daily cap, daily reminder / streak-at-risk, pg_cron schedule, deep-link taps — all T-050C. |
