...
|----|--------|-------|------|-------|
| T-053 | DOING | mvp | UI/UX refinement batch (titles, reactions, comment likes, leaderboard, streak, refresh, keyboard) | A done (W-034); B–E to go. See backlog row + HANDOFF 2026-06-11. |
...
| T-053 | DOING | UI/UX refinement batch (8 items). Sliced A–E (see HANDOFF 2026-06-11). |
| T-053-A | DONE | Submission titles end-to-end. `submissions.title text NOT NULL` (CHECK 1..80) with legacy-row backfill of `'Day N'`; 5 RPCs recreated to thread/return the new column (`submit_proof`, `redact_my_submission`, `list_challenge_submissions`, `get_submission_with_author`, `get_my_today_submission`); `Submission` entity + `proofInput` zod + queue payload all carry title; `ProofComposer` got a required Title input above the photo (renamed its existing screen-heading prop `title`→`screenTitle`); challenge detail row uses title as primary line + `<author> · Day N` subline; submission detail reordered to Title → Author w/ Avatar + Day + datetime → Photo → Description → Reactions → Comments; verify screen surfaces title at top + Day subline; profile rows use submission title + date-only (no clock) + group name. Migration `20260608000000_submission_titles.sql` (W-034 — USER must apply). |
| T-053-B | TODO | Comment likes + merged composer arrow-send. New `comment_likes` table (W-035) + `like_comment` / `unlike_comment` / `list_comment_likers` RPCs; heart icon on each comment row (counter shown only when > 0; long-press → list of likers); merge "Add a comment…" input + "Post" button into one row with an arrow-icon send affordance. |
| T-053-C | TODO | Free-form emoji reactions + reactor popover. Add `rn-emoji-keyboard` dep; loosen server-side reaction enum if any (potential W-036); rewrite `ReactionBar` to show current reactions + a `(+)` button opening an inline preset popover that itself has a `(+)` opening the full system-style emoji picker; long-press an existing reaction → list of users who reacted with it. New `list_reactors` RPC. |
| T-053-D | TODO | Other-user profile route + leaderboard polish. New `app/user/[id].tsx` read-only profile with avatar / name / bio / heatmap / recent submissions; new `get_public_profile` RPC widening the row enough for the heatmap + recent submissions under existing RLS; group leaderboard rewrite — avatars on the left of name (right of rank #), redesigned crown to the right of nickname, tap → user profile; transfer-leadership moves into the group 3-dot sheet (open menu → "Transfer leadership" → modal member picker). May need a small RPC widening for `group_leaderboard` to include `avatar_url` (W-037). |
| T-053-E | TODO | Polish + streak + keyboard. Pull-to-refresh on Home + Profile tabs; aggregated "Current streak" + "Best streak" stat tiles on Profile (new `get_my_streak_aggregate` RPC, W-038); iOS keyboard "Done" affordance via a shared `inputAccessoryView` component above the keyboard. |
