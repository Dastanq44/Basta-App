...
| T-074 | TODO | mvp | **Global tab** (feed of verified+public submissions) — UNBLOCKED by T-075. Query gated by `is_submission_globally_visible` SERVER-side (never client-filter). Global v1 = public verified posts only; link profiles/challenges/groups from posts, not separate discovery tabs. Do NOT expose invite codes. |
